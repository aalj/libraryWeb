...
public class SearchLectures {
	/**
	 * 
	 * SearchNewsByLimit:(通过分页查询的方式按照时间排序的方式查询讲座)<br/>
	 *
	 * @param @param
	 *            page
	 * @param @param
	...
	public LecturesTable SearchLecturesByLecId(int lec_id) {
		PreparedStatement prepare = null;
		ResultSet query = null;
		Connection conn = DBconnection.getConnection();
	...
	/**
	 * 
	 * setLecturesRegistration:(查询报名的讲座)<br/>
	 *
	 * @param @param
	 *            lec_id
	 * @param @param
	 ...
	 *             CodingExample Ver 1.1
	 */
	public List<LecturesRegistrationTable> getLecturesRegistrationb(int stu_id,int page) {
		LoginDao loginDao = new LoginDao();
		PreparedStatement prepare = null;
		ResultSet query = null;
		Connection conn = DBconnection.getConnection();
		String sql = "select * from lectures_registration_table where stu_id=? order by create_time  limit ?,?;";
		// String sql = "insert into lectures_registration_table
		// (lec_id,stu_id,create_time) values(?,?,?); ";
		try {
			...
			query = prepare.executeQuery();
			List<LecturesRegistrationTable> list = new ArrayList<>();
			LecturesRegistrationTable lecturesRegistrationTable = null;

			while (query.next()) {

				lecturesRegistrationTable = new LecturesRegistrationTable();
				lecturesRegistrationTable.setLere_id(query.getInt("lere_id"));
				lecturesRegistrationTable.setLecturesTable(SearchLecturesByLecId(query.getInt("lec_id")));
				lecturesRegistrationTable.setStudent(loginDao.MLogin(query.getString("stu_id")));
				lecturesRegistrationTable.setCreate_time(query.getTimestamp("create_time"));
				list.add(lecturesRegistrationTable);
			}
